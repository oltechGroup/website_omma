...
export default function AboutPage() {
  ...
  // Nota: El stagger (ej. lg:mt-12) ya está configurado para aplicar solo en pantallas grandes
  ...
  return (
    <div className="w-full min-h-screen bg-white flex flex-col font-sans relative box-border pt-20 md:pt-24">
      ...
      <div className={`w-full transition-all duration-300 z-[999] fixed top-0 left-0 right-0 py-3 md:py-4 ${navBgClass}`}>
        <div className="w-full max-w-7xl mx-auto flex items-center justify-between px-4 md:px-6 border-b border-gray-100 pb-2 md:pb-3">
          ...
          <nav className={`flex items-center space-x-4 sm:space-x-8 md:space-x-12 font-medium text-[13px] sm:text-sm md:text-base ${textColorClass} transition-colors duration-300`}>
            ...
            {/* SELECTOR DE IDIOMA (Oculto en móviles como ya estaba previsto) */}
      ...
      <section className="max-w-7xl mx-auto w-full px-6 py-16 md:py-32 flex flex-col md:flex-row items-center md:items-start gap-8 md:gap-24 box-border text-center md:text-left">
        {/* Título Gigante */}
        {/* Ajuste de tamaño en móvil (!text-4xl) */}
        <div className="w-full md:w-1/2 flex flex-col">
          <h1 className="!text-4xl sm:!text-5xl md:!text-7xl lg:!text-[5.5rem] !font-extrabold !text-[#32453F] leading-[1.1] tracking-tighter block mb-4 md:mb-6">
            {t.hero.title} <br className="hidden md:block" />
            {t.hero.subtitle}
          </h1>
        </div>
        {/* Textos Misión y Visión */}
        <div className="w-full md:w-1/2 flex flex-col justify-center gap-6 md:gap-10 md:pt-4">
          <p className="text-sm sm:text-base md:text-lg text-[#5E706B] font-light leading-relaxed">
            {t.mission.text}
          </p>
          <p className="text-sm sm:text-base md:text-lg text-[#5E706B] font-light leading-relaxed">
            {t.vision.text}
      ...
      <section className="max-w-6xl mx-auto w-full px-4 md:px-8 mb-20 md:mb-32 box-border">
        <div className="grid grid-cols-1 md:grid-cols-2 relative">
          ...
          <div className="border-b border-[#D9E3E0] md:border-r p-8 md:p-14 flex flex-col items-center justify-center">
            <h2 className="text-2xl md:text-3xl font-extrabold text-[#526B63] mb-8 text-center">{t.values.title}</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-md">
              <div className="border border-gray-400 rounded-full py-3 px-4 text-center text-[#7C8B87] font-medium text-xs sm:text-sm md:text-base hover:border-[#32453F] transition-colors cursor-default">{t.values.items.innovation}</div>
              <div className="border border-gray-400 rounded-full py-3 px-4 text-center text-[#7C8B87] font-medium text-xs sm:text-sm md:text-base hover:border-[#32453F] transition-colors cursor-default">{t.values.items.excellence}</div>
              <div className="border border-gray-400 rounded-full py-3 px-4 text-center text-[#7C8B87] font-medium text-xs sm:text-sm md:text-base hover:border-[#32453F] transition-colors cursor-default">{t.values.items.trust}</div>
              <div className="border border-gray-400 rounded-full py-3 px-4 text-center text-[#7C8B87] font-medium text-xs sm:text-sm md:text-base hover:border-[#32453F] transition-colors cursor-default">{t.values.items.accessibility}</div>
              <div className="border border-gray-400 rounded-full py-3 px-4 text-center text-[#7C8B87] font-medium text-xs sm:text-sm md:text-base hover:border-[#32453F] transition-colors cursor-default">{t.values.items.sustainability}</div>
              <div className="border border-gray-400 rounded-full py-3 px-4 text-center text-[#7C8B87] font-medium text-xs sm:text-sm md:text-base hover:border-[#32453F] transition-colors cursor-default">{t.values.items.service}</div>
            </div>
          </div>
          ...
          <div className="border-b border-[#D9E3E0] p-8 md:p-14 flex flex-col items-center justify-center text-center">
            <p className="italic text-[#465A54] text-base md:text-xl font-light leading-relaxed mb-8 max-w-lg">
              "{t.positioning}"
            </p>
            <div className="w-24 md:w-32 h-[1px] bg-[#465A54]"></div>
          </div>

          {/* CUADRANTE 3: Perfil Empresarial (Abajo Izquierda) */}
          {/* Borde derecho solo en md: para evitar la línea suelta en móvil */}
          <div className="border-b md:border-b-0 md:border-r border-[#D9E3E0] p-8 md:p-14 flex flex-col items-center text-center justify-center">
            <h3 className="font-extrabold text-[#526B63] text-lg md:text-xl mb-3">{t.profile.title}</h3>
            <p className="text-[#5E706B] font-light text-sm md:text-base mb-8 md:mb-10 max-w-sm">
              {t.profile.text}
            </p>
            <h3 className="font-extrabold text-[#526B63] text-lg md:text-xl mb-3">{t.profile.subtitle}</h3>
          ...
          <div className="p-8 md:p-14 flex flex-col items-center justify-center">
            <h2 className="text-2xl md:text-3xl font-extrabold text-[#526B63] mb-6 md:mb-8 text-center">{t.experience.title}</h2>
            <div className="flex flex-col gap-3 w-full max-w-md">
              {t.experience.items.map((item, idx) => (
                <div key={idx} className="border border-gray-400 rounded-2xl p-4 text-[11px] sm:text-xs md:text-sm text-[#7C8B87] font-light hover:border-[#32453F] hover:shadow-sm transition-all cursor-default">
                  {item}
                </div>
              ))}
            </div>
      ...
      {/* ========================================= */}
      <section className="max-w-7xl mx-auto w-full px-6 mb-24 md:mb-48 box-border">
        <h2 className="text-3xl md:text-5xl font-extrabold text-[#32453F] text-center mb-10 md:mb-24">{t.clients.title}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 pb-12">
          {t.clients.list.map((client, index) => {
            ...
            return (
              <div 
                key={index} 
                className={`rounded-[2.5rem] p-6 md:p-8 flex flex-col ${style.bg} transition-transform hover:-translate-y-2 duration-300 ${style.stagger} self-stretch lg:self-start`}
              >
                ...
                <div className="mb-6 md:mb-8 flex justify-start">
                  {client.link ? (
                    <a 
                      ...
                      target="_blank" 
                      rel="noreferrer" 
                      className={`inline-block px-4 md:px-5 py-2 rounded-full border ${style.pillBorder} text-[10px] md:text-xs font-semibold uppercase tracking-wider hover:opacity-70 transition-opacity`}
                    >
                      {t.clients.visit}
                    </a>
                  ) : (
                    <span className={`inline-block px-4 md:px-5 py-2 rounded-full border ${style.pillBorder} text-[10px] md:text-xs font-semibold uppercase tracking-wider opacity-50`}>
                      {t.clients.noWebsite}
                    </span>
                  )}
                </div>
                ...
                {/* Imagen del Cliente */}
                <div className="w-full h-40 md:h-52 rounded-[2rem] overflow-hidden mb-6 md:mb-10 bg-white/50 shadow-sm">
                  <img 
                    src={client.image} 
                    alt={client.name} 
                ...
                <div className="mt-auto flex flex-col items-start">
                  <a 
                    ...
                    target="_blank" 
                    rel="noreferrer" 
                    className="mb-4 md:mb-5 w-10 md:w-12 h-10 md:h-12 flex items-center justify-center rounded-full bg-[#000000] text-white hover:bg-gray-600 transition-colors shadow-lg"
                    title={client.phone}
                  >
                    <FaWhatsapp size={18} className="md:w-5 md:h-5" />
                  </a>
                  <h3 className={`text-xl md:text-3xl font-bold mb-2 md:mb-3 leading-tight tracking-tight ${style.text}`}>
                    {client.name}
                  </h3>
                  <p className={`text-xs md:text-base font-light leading-relaxed max-w-sm ${style.descText}`}>
                    {client.address}
                  </p>
                </div>
              </div>
      ...
      <footer className="w-full bg-[#203C46] text-white py-12 md:py-16 px-6 md:px-12 mt-auto box-border block">
        ...
        <div className="max-w-6xl mx-auto mt-12 md:mt-16 pt-8 border-t border-white/10 flex flex-col md:flex-row items-center justify-between opacity-70 text-sm gap-4 md:gap-0">
          <p className="text-center md:text-left">© {new Date().getFullYear()} OMMA Group LLC. {t.footer.rights.split('©')[1]?.substring(5) || "Todos los derechos reservados."}</p>
