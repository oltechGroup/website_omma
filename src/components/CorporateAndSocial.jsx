    ...
    <section className="w-full bg-[#FAFAFA] py-20 px-4 flex flex-col items-center overflow-hidden">
      ...
      <style>{`
        @keyframes marquee {
          ...
          100% { transform: translateX(-50%); } /* Se mueve el 50% porque la lista está duplicada */
        }
        .animate-marquee {
          display: flex;
          width: max-content;
        ...
        /* Pausar carrusel al pasar el ratón */
      ...
      <div className="max-w-6xl w-full flex flex-col gap-16">
            ...
            {t.corporate.values.map((item, index) => (
              <div key={index} className="flex items-start gap-4">
                ...
                <img 
                  ...
                  alt="Icono OMMA" 
                  className="w-6 h-6 mt-1 flex-shrink-0 object-contain drop-shadow-sm" 
                />
                <div>
                  <h3 className="text-lg font-bold text-[#4A4A4A] mb-1">{item.title}</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full">
          ...
          <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.08)] p-4 overflow-hidden flex justify-center h-[600px] border border-gray-100">
            <iframe 
        ...
        <div className="w-full mt-10">
          ...
          <div className="w-full overflow-hidden marquee-container relative py-4">
            ...
            <div className="absolute top-0 left-0 w-20 h-full bg-gradient-to-r from-[#FAFAFA] to-transparent z-10 pointer-events-none"></div>
              ...
              {[...translatedReviews, ...translatedReviews].map((review, index) => (
                ...
                >
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      <h4 className="font-bold text-gray-800 text-sm truncate w-48">{review.name}</h4>
                      <p className="text-xs text-gray-400">{review.details}</p>
                    </div>
                  </div>
                  <div className="flex text-[#FABB05] text-sm mb-1 gap-0.5">
                    <FaStar /><FaStar /><FaStar /><FaStar /><FaStar />
                  </div>
                  <p className="text-xs text-gray-400 mb-3">{review.time}</p>
                  ...
                  <div className="flex items-center gap-4 text-gray-400 mt-auto">
                    <button className="flex items-center gap-1 hover:text-red-500 transition-colors"><FaRegHeart /> <span className="text-xs">{(index % 5) + 1}</span></button>
